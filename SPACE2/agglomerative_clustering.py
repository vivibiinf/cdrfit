...
def agglomerative_clustering(files, imgt=None, cdr_lens=None,  selection=reg_def["CDR_all"], anchors=reg_def["fw_all"], cutoff=1.25, n_jobs=-1):
    ...
    :return:
    """
    clustering_algorithm = AgglomerativeClustering(n_clusters=None, metric='precomputed', distance_threshold=cutoff,
                                                   linkage='complete')
    final_clustering = cluster_with_algorithm(clustering_algorithm, files, imgt, cdr_lens, selection=selection, anchors=anchors, n_jobs=n_jobs)
